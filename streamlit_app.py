...
# 1. Initialize Session State (Restored Chat History)
...
if not tab_df.empty:
    ...
    # --- RESTORED: SIDEBAR GROQ CHAT ---
    st.sidebar.divider()
    st.sidebar.subheader("💬 Ask Groq AI")
    user_q = st.sidebar.text_input("Ask about this data:", key="user_input")
    
    if user_q:
        with st.sidebar:
            with st.spinner("Groq is thinking..."):
                # Use current tab data to context-aware answers
    ...
    # --- KEYWORD DEEP-DIVE & TRENDS ---
    st.subheader("Keyword Performance & Projections")
    show_forecast = st.checkbox("🔮 Show Scikit-Learn Projections", value=True)
    ...
    if metric_name_col and value_col and date_col in tab_df.columns:
        ...
        for loc in loc_list:
            ...
            for kw in region_keywords:
                ...
                with st.expander(f"Trend for: {kw}", expanded=(loc == 'Germany')):
                    ...
                    with col1:
                        ...
                        if show_forecast and len(kw_data) >= 2:
                            ...
                            if forecast is not None:
                                fig.add_trace(go.Scatter(
                                    ...
                                    line=dict(color='rgba(255,255,255,0)'), name='Confidence', showlegend=False
                                ))
                                fig.add_trace(go.Scatter(
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"main_ch_{loc}_{c_idx}")
                    
                    with col2:
                        st.write("**Data**")
                        table_data = kw_data[['dt', value_col]].copy()
                        table_data['dt'] = table_data['dt'].dt.strftime('%b %Y')
                        st.dataframe(table_data, hide_index=True, use_container_width=True, key=f"tbl_view_{loc}_{c_idx}")
