...
if not tab_df.empty:
    ...
    is_ranking = value_col and 'POSITION' in value_col.upper()aimport streamlit as st
import pandas as pd
...
# --- 2. Data Loading (MUST HAPPEN FIRST) ---
try:
            ...
            for kw in region_metrics:
                ...
                with st.expander(f"Data for: {kw} in {loc}", expanded=(loc == 'Germany')):
                    ...
                    with col1:
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"trend_{c_idx}")
                    ...
                    with col2:
                        ...
                        st.download_button("Download CSV", csv, f"{kw}_{loc}.csv", key=f"dl_{c_idx}")
else:
    st.info("Please wait for data to load or select a different section.")
